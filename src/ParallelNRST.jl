...
function parallel_run(
    ...
    nss = [copy(ns) for _ in 1:ntours]                    # pre-allocate samplers
    res = [NRSTTrace(T,ns.np.N,TF) for _ in 1:ntours]     # pre-allocate storage for results
    ...
    @sync for t in 1:ntours                               # @sync tells the loop to end only when all @async operations inside end
        tsk = Threads.@spawn begin
            tour!(nss[t], res[t]; kwargs...)              # run a full tour with a local temp copy of ns. copying is fast relative to cost of a tour, and size(ns) is ~ size(ns.x)                                   # writing to separate locations in a common vector is fine. see: https://discourse.julialang.org/t/safe-loop-with-push-multi-threading/41892/6, and e.g. https://stackoverflow.com/a/8978397/5443023
